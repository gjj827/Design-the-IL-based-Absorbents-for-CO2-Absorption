...
class GCNet(torch.nn.Module):
    def __init__(self, v_in, conv_dim, net_dim, n_extra_inputs):
        ...
        self.gat = GATConv(conv_dim, conv_dim)
...
possible_atom_list = ['Al', 'B', 'Br', 'C', 'Cl', 'F', 'I', 'N', 'O', 'P', 'S', 'Si']
...
possible_formal_charge = [0, 1, -1,3]
...
def sys2graph(df, mol_column_1, mol_column_2,single_system=False):
    ...
    for comp1, comp2, T, P, HB, ap_Cation, ap_Anion, bp_Cation, bp_Anion, topopsa_Cation, topopsa_Anion in tqdm(zip(c1, c2, Ts, Ps, HBs, ap_Cations, ap_Anions, bp_Cations, bp_Anions, topopsa_Cations, topopsa_Anions)):
...
new_df = pd.read_csv('screening_process/co2_pre.csv')
...
scaler = joblib.load('Viscosity/scaler.pkl')
new_features = ['Temperature(K)', 'Pressure(bar)','ap_Cation','ap_Anion','bp_Cation','bp_Anion',
...
norm_feature_names = ['T_norm', 'P_norm','ap_Cation_norm','ap_Anion_norm','bp_Cation_norm','bp_Anion_norm',
...
new_dataloader = DataLoader(new_df['g_ILs'].tolist(), batch_size=32, shuffle=False, drop_last=False)
device = 'cuda' if torch.cuda.is_available() else 'cpu'
model = GCNet(34, 100, 100, 9).to(device)
checkpoint = torch.load('Viscosity/Vis_DG_GCN_GAT.pth')
...
new_df['predicted_VIS'] = y_hat_new
columns_to_save = [
    'Cation_SMILES','Cation_PubChemID','Anion_SMILES','Anion_PubChemID', 
    'IL_SMILES', 'Temperature(K)', 'Pressure(bar)', 'predicted_XCO2','predicted_VIS'
]
new_df[columns_to_save].to_csv('vis_pre.csv', index=False)
print("The results have been stored in vis_pre.csv!")