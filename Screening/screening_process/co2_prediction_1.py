...
class GCNet(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        x = F.sigmoid(self.gat(x, edge_index))
...
new_df = pd.read_csv('screening_data/screening_dataset.csv')
...
scaler = joblib.load('CO2_solubility/scaler.pkl')
...
model = GCNet(31, 9, 9, 9,4).to(device) 
checkpoint = torch.load('CO2_solubility/CO2_DG_GCN_GAT.pth')
...
new_df['predicted_XCO2'] = y_hat_new
columns_to_output = [
    'Cation_SMILES','Cation_PubChemID','Anion_SMILES','Anion_PubChemID', 
    'IL_SMILES', 'Temperature(K)', 'Pressure(bar)', 'predicted_XCO2'
]
new_df[columns_to_output].to_csv('co2_pre.csv', index=False)
print("The results have been stored in co2_pre.csv!")