...
class GINNet(torch.nn.Module):
    def __init__(self, v_in, conv_dim, net_dim, n_extra_inputs):
        ...
        nn1 = nn.Sequential(nn.Linear(v_in, conv_dim), nn.ReLU(), nn.Linear(conv_dim, conv_dim))
        nn2 = nn.Sequential(nn.Linear(conv_dim, conv_dim), nn.ReLU(), nn.Linear(conv_dim, conv_dim))
...
model = GINNet(31,9, 9, 9).to(device)
...
print("The relevant parameters have been saved to the path: CO2_DG_GIN.pth!")
train_mse = np.sqrt(np.array(train_loss_save))
test_mse = np.sqrt(np.array(test_loss_save))

model = GINNet(31,9, 9, 9).to(device)
checkpoint = torch.load('CO2_DG_GIN.pth')
