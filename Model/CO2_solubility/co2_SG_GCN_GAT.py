...
df['g_ILs'] = comp_to_graph(df, 'mol_SMILES', 'Xco2(mole fraction)')
...
model = GCNet(31, 9, 9, 2,4).to(device)
...
print("The relevant parameters have been saved to the path : CO2_SG_GNN.pth!")
train_mse = np.sqrt(np.array(train_loss_save))
test_mse = np.sqrt(np.array(test_loss_save))
model = GCNet(31, 9, 9, 2,4).to(device)
checkpoint = torch.load('CO2_SG_GNN.pth')
