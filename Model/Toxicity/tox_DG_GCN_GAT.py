...
features = ['ap_Cation','ap_Anion','bp_Cation','bp_Anion',
...
def sys2graph(df, mol_column_1, mol_column_2, target, single_system=False):
    ...
    for y, comp1, comp2, HB, ap_Cation, ap_Anion, bp_Cation, bp_Anion, topopsa_Cation, topopsa_Anion in tqdm(zip(ys, c1, c2,  HBs, ap_Cations, ap_Anions, bp_Cations, bp_Anions, topopsa_Cations, topopsa_Anions), total=len(ys)):
...
df['g_ILs'] = sys2graph(df, 'mol_Cation', 'mol_Anion', 'logEC50')
train_df = df[df['Split'] == 'Training']
test_df = df[df['Split'] == 'Testing']
train_dataloader = DataLoader(train_df['g_ILs'].tolist(), batch_size=8, shuffle=False, drop_last=False)
test_dataloader = DataLoader(test_df['g_ILs'].tolist(), batch_size=8, shuffle=False, drop_last=False)
...
if __name__ == "__main__":
    ...
    print("The relevant parameters have been saved to the path: Tox_DG_GCN_GAT.pth!")
    train_mse = np.sqrt(np.array(train_loss_save))
    test_mse = np.sqrt(np.array(test_loss_save))
    model = GCNet(31, 100, 100, 7,4).to(device)
    checkpoint = torch.load('Tox_DG_GCN_GAT.pth')
