...
df = pd.read_csv('Data/clean_data/vis_iqr_data.csv')
...
features = ['Temperature(K)', 'Pressure(bar)','ap_Cation','ap_Anion','bp_Cation','bp_Anion',
            'topopsa_Cation','topopsa_Anion','hb_Cation','hb_Anion','inter_hb_sites']
X_train = train_df[features].values
y_train = train_df['LOG10(VISCOSITY)(mPa.s)'].values
X_test = test_df[features].values
y_test = test_df['LOG10(VISCOSITY)(mPa.s)'].values
...
df['g_ILs'] = sys2graph(df, 'mol_Cation', 'mol_Anion', 'LOG10(VISCOSITY)(mPa.s)')
...
if __name__ == "__main__":
    ...
    print("The relevant parameters have been saved to the path: Vis_DG_GCN_GAT.pth!")
    train_mse = np.sqrt(np.array(train_loss_save))
    test_mse = np.sqrt(np.array(test_loss_save))
    model = GCNet(34, 100, 100, 9).to(device) 
    checkpoint = torch.load('Vis_DG_GCN_GAT.pth')
