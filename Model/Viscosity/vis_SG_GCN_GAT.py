...
def comp_to_graph(df, mol_column, target):
    ...
    for y, comp, T, P in tqdm(zip(ys, comps, Ts, Ps), total=len(ys)):
...
df['g_ILs'] = comp_to_graph(df, 'mol_SMILES', 'LOG10(VISCOSITY)(mPa.s)')
...
model = GCNet(34, 20, 20, 2).to(device)
...
torch.save({
    ...
    'Viscosity_SG_GCN_GAT.pth') 
print("The relevant parameters have been saved to the path : Viscosity_SG_GCN_GAT.pth!")
train_mse = np.sqrt(np.array(train_loss_save))
test_mse = np.sqrt(np.array(test_loss_save))
model = GCNet(34, 20, 20, 2).to(device)
checkpoint = torch.load('Viscosity_SG_GCN_GAT.pth')
